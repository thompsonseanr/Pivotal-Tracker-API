...
class PivotalTrackerAPI{
	...
	/* Diagnostic Failure Email */
		...
		$bcc = array(
			"sean.thompson@inenvi.com"
		);
		
		$attachments = array();
		array_push($attachments, $this->userUploadFileA); 
		array_push($attachments, $this->userUploadFileB);
		$headers = array( "Content-Type: text/html; charset=UTF-8", "From: ". $from_email, "BCC: ". implode(",", $bcc));  //, "From: ". $from_email, "BCC: developers@inenvi.com" 
		$subject ="***STORY CREATION FAILURE NOTICE***   " . 'New ' . $this->createStoryArray['name'] . ' in ' . $project_id_inenvi;
		$content ="<b><u>The Following Story Was Not Submitted To Pivotal Tracker:</u></b>" . "<br/><br/><br/>" . "<b>Story Title: </b>" . $this->createStoryArray['name'] . "<br/><br/>" . '<b>Description: </b>' . $this->createStoryArray['description'] . "<br/><br/>" . '<b>Story Type: </b>Feature' . "<br/><br/>" . '<b>Requested By: </b>' . $this->userApi->user_firstname . ' ' . $this->userApi->user_lastname . "<br/><br/>" . '<b>Current Status: </b>' . ucwords($this->createStoryArray['current_state']) . "<br/><br/>" . '<b>Project Queue: </b>' . ucwords($this->conditionA) . "<br/><br/>" .'<b>Created On: </b>' . date("m/d/y") . "<br/><br/>" . '<b>Urgency of Request: </b>' . $this->requestUrgencyForm;
		$sent = wp_mail($to, $subject, $content, $headers, $attachments); // $headers, $attachments
		return $sent;
	}
	

		$this->jsonDecodeStoryResults = json_decode($this->storyCreatePostResults, true);
		if($this->jsonDecodeStoryResults['project_id'] == '1509754'){
			$project_id_inenvi = 'Website New Development';
		}
		elseif($this->jsonDecodeStoryResults['project_id'] == '1508596'){
			$project_id_inenvi = 'Website Support';
		}
		elseif($this->jsonDecodeStoryResults['project_id'] == '1508532'){
			$project_id_inenvi = 'Enterprise New Development';
		}
		elseif($this->jsonDecodeStoryResults['project_id'] == '1508660'){
			$project_id_inenvi = 'Enterprise Support';
		}
		elseif($this->jsonDecodeStoryResults['project_id'] == '1509792'){
			$project_id_inenvi = 'Operations Support';
		}
		...
		if ($project_id_inenvi == 'Website New Development'){
			$to = array(
			"sean.thompson@inenvi.com"
			);
		}
		
		if ($project_id_inenvi == 'Website Support'){
			$to = array(
			"sean.thompson@inenvi.com"
			);
		}
		
		if ($project_id_inenvi == 'Enterprise New Development' || $project_id_inenvi == 'Enterprise Support'){
			$to = array(
			"sean.thompson@inenvi.com"
			);
		}
		
		if ($project_id_inenvi == 'Operations Support'){
			$to = array(
			"sean.thompson@inenvi.com"
			);
		}
		...
		$headers = array( "Content-Type: text/html; charset=UTF-8", "From: ". $from_email); // . $from_email bloginfo('admin_email')
		$headersA = array( "Content-Type: text/html; charset=UTF-8", "From: sean.thompson@inenvi.com"); 
		$subject = 'New ' . $this->jsonDecodeStoryResults['name'] . ' in ' . $project_id_inenvi;
		$subjectA = 'Email Send Failure: Invalid Email Address';
		$content = "<b>Story Title: </b>" . $this->jsonDecodeStoryResults['name'] . "<br/><br/>" . '<b>Description: </b>' . $this->jsonDecodeStoryResults['description'] . "<br/><br/>" . '<b>Story Type: </b>' . ucwords($this->jsonDecodeStoryResults['story_type']) . "<br/><br/>" . '<b>Notes Created By: </b>' . $this->userApi->user_firstname . ' ' . $this->userApi->user_lastname . "<br/><br/>" . '<b>Current Status: </b>' . ucwords($this->jsonDecodeStoryResults['current_state']) . "<br/><br/>" . '<b>Story Id: </b>' . $this->jsonDecodeStoryResults['url'] . "<br/><br/>" . '<b>Project Queue: </b>' . $project_id_inenvi . "<br/><br/>" .'<b>Created On: </b>' . str_replace($createReplaceArray, $createSubArray,$this->jsonDecodeStoryResults['created_at']) . "<br/><br/>" . '<b>Urgency of Request: </b>' . $this->requestUrgencyForm;
		$sent = wp_mail($to, $subject, $content, $headers, $attachments); // $headers, $attachments
